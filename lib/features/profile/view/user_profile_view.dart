...
class UserProfileView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final PublicProfileController controller = Get.put(PublicProfileController(targetUserId: userId), tag: userId); // 🔥 Tag added to prevent mixup
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _showOptionsBottomSheet(context, userId, userName, safetyController),
          )
        ],
      ),
      body: Obx(() {
        ...
        return Stack(
          children: [
            Container(decoration: const BoxDecoration(gradient: LinearGradient(colors: [Color(0xFF0F0518), Color(0xFF1A0B2E), Color(0xFF0F0518)], begin: Alignment.topCenter, end: Alignment.bottomCenter))),
            Positioned(
              top: -50, right: -50,
              child: Container(
                width: 200, height: 200, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.pinkAccent.withOpacity(0.1)),
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.pinkAccent, width: 2), boxShadow: [BoxShadow(color: Colors.pinkAccent.withOpacity(0.4), blurRadius: 20)]),
                      child: CircleAvatar(
                        radius: 55, backgroundColor: const Color(0xFF1A1A2E),
                        backgroundImage: avatarUrl.isNotEmpty ? NetworkImage(avatarUrl) : null,
                        child: avatarUrl.isEmpty ? Text(name.substring(0, 1).toUpperCase(), style: const TextStyle(fontSize: 40, color: Colors.white, fontWeight: FontWeight.bold)) : null,
                      ),
                    ),
                    const SizedBox(height: 15),
                    Text(name, style: const TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 5),
                    Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: Colors.white.withOpacity(0.1), borderRadius: BorderRadius.circular(10)), child: Text("ID: $userId", style: const TextStyle(color: Colors.white70, fontSize: 13))),
                    const SizedBox(height: 15),
                    Padding(padding: const EdgeInsets.symmetric(horizontal: 40), child: Text(bio, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white54, fontSize: 14))),
                    ...
                    Row(
                      ...
                      children: [
                        _buildStatColumn(
                          ...
                          onTap: () => Get.to(() => FollowersFollowingView(userId: userId, type: 'followers')),
                        ),
                        _buildStatColumn(
                          "Following",
                          controller.followingCount.value.toString(),
                          onTap: () => Get.to(() => FollowersFollowingView(userId: userId, type: 'following')),
                        ),
                    ...
                    Row(
                      ...
                      children: [
                        GestureDetector(
                          ...
                          child: Container(
                            width: 140, padding: const EdgeInsets.symmetric(vertical: 14),
                            decoration: BoxDecoration(
                              gradient: LinearGradient(colors: controller.isFollowing.value ? [Colors.grey[800]!, Colors.grey[700]!] : [Colors.pinkAccent, Colors.deepPurpleAccent]),
                            ...
                            child: Center(child: Text(controller.isFollowing.value ? "Following" : "Follow", style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold))),
                          ),
                        ),
                        ...
                        GestureDetector(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(color: Colors.white.withOpacity(0.1), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.white.withOpacity(0.3))),
                            child: const Center(child: Icon(Icons.chat_bubble_outline, color: Colors.white)),
                          ),
                        ),
                      ],
                    ),
  ...
  void _showReportDialog(BuildContext context, String targetId, String targetName, SafetyController safetyController) {
    final List<String> reportReasons = ['Nudity or sexually explicit content', 'Hate speech or symbols', 'Violence or dangerous behavior', 'Bullying or harassment', 'Scam or fraud', 'Spam'];
                    ...
                    child: Column(
                      ...
                      children: reportReasons.map((reason) {
                        return RadioListTile<String>(
                          ...
                          value: reason, groupValue: selectedReason,
                  ...
                  actions: [
                    ...
                    Obx(() => ElevatedButton(
                      ...
                      onPressed: safetyController.isProcessing.value ? null : () {
                        safetyController.submitReport(reportedUserId: targetId, reason: selectedReason, details: 'Reported from User Profile View', source: 'user_profile');
                      },
